...
const ForumPostRow = React.memo(function ForumPostRow({ post, canDelete, onToggleLike, onShare, onDelete }: ForumPostRowProps) {
                ...
                <div className="flex-1 min-w-0 space-y-2">
                    ...
                    <div className="flex items-center justify-between gap-2 flex-wrap">
                        <div className="flex items-center gap-1.5 flex-wrap">
                            ...
                            {post.is_anonymous ? (
                                <Badge size="xs" uppercase tone="bg-primary/10 text-primary border border-primary/20">
                                    Ẩn danh
                                </Badge>
                            ) : (
                                <Badge
...
const SearchBox = React.memo(function SearchBox({ value, onCommit, onClear }: SearchBoxProps) {
...
export default function ForumPage() {
    ...
    // Fetch phân trang: offset = 0 → thay danh sách, offset > 0 → nối thêm trang mới
    const fetchPosts = useCallback(async (query = '', offsetValue = 0) => {
        const epoch = ++fetchEpochRef.current;
        if (offsetValue === 0) setLoading(true); else setLoadingMore(true);
        try {
            const params = [`limit=${PAGE_SIZE}`, `offset=${offsetValue}`];
            ...
            if (res.success) {
                // Contract mới: có limit → data = {items, total, limit, offset}
                const items: Post[] = Array.isArray(res.data?.items) ? res.data.items : [];
                const total: number = typeof res.data?.total === 'number' ? res.data.total : 0;
                setPosts(prev => (offsetValue === 0 ? items : [...prev, ...items]));
                setHasMore(offsetValue + items.length < total);
            }
        } catch (err) {
    ...
    // Đổi chủ đề/từ khoá → quay về trang đầu và thay toàn bộ danh sách.
    // Gọi qua setTimeout(0) để setState không chạy đồng bộ trong thân effect;
    // clearTimeout cũng hủy fetch pending khi filter đổi nhanh liên tiếp
    useEffect(() => {
        const t = setTimeout(() => fetchPosts(search, 0), 0);
        return () => clearTimeout(t);
    }, [search, selectedCat, fetchPosts]);

    // Infinite scroll: observer thật trên sentinel đáy feed (trước đây chỉ là scaffold chết)
    const loadMore = useCallback(() => {
        if (loading || loadingMore || !hasMore) return;
        fetchPosts(search, posts.length);
    }, [loading, loadingMore, hasMore, search, posts.length, fetchPosts]);
    ...
    const toggleLike = useCallback(async (postId: string, e: React.MouseEvent) => {
        ...
        try {
            ...
            fetchPosts(search, 0); // Hoàn tác khi lỗi: reload về trang đầu như behaviour cũ
        }
    }, [user, router, showToast, fetchPosts, search]);

    const handleShare = useCallback((postId: string, e: React.MouseEvent) => {
        e.preventDefault();
        e.stopPropagation();
        ...
        <div className="min-h-screen bg-background text-on-surface pb-24">
            ...
            <div className="max-w-2xl mx-auto px-4 pt-6 pb-2">
                <div className="flex items-center justify-between gap-4">
                    ...
                    {/* Search Input */}
                    <SearchBox value={search} onCommit={setSearch} onClear={handleSearchClear} />
                </div>
            </div>
                        ...
                        <div className="divide-y divide-outline-variant/20">
                            {posts.map((post) => {
                                const canDelete = Boolean(
                                    post.is_owner ||
                                    (user && post.author.id && String(post.author.id) === String(user.id)) ||
