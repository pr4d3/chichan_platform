...
export function HeroTypingTitle({ className = "" }: HeroTypingTitleProps) {
  ...
  useEffect(() => {
    ...
    const step = () => {
      ...
      if (phase === "delete") {
        chars -= 1;
        el.textContent = keyword.slice(0, chars);
        if (chars > 0) {
  ...
  return (
    <h1 className={`flex flex-col gap-1.5 text-on-surface select-none ${className}`}>
      ...
      <div className="text-2xl sm:text-3xl md:text-4xl font-extrabold tracking-tight h-[1.35em] flex items-center text-primary">
        ...
        `}</style>
        <span
          className="inline-block w-[3.5px] h-[0.85em] bg-primary ml-1.5 rounded-full animate-[caret-blink_0.9s_ease-in-out_infinite]"
          aria-hidden="true"
        />
      </div>
    </h1>
